...
# Resolve absolute path to local_data folder
data_path = os.path.join(os.path.dirname(__file__), "local_data")

filesystem_resource = filesystem(
    bucket_url=data_path,
    file_glob="*.parquet"   # or "**/*.parquet" for nested folders
)
...
def run_source():
    pipeline = dlt.pipeline(
        pipeline_name="parquet_pipeline"
        , destination="duckdb"
    )
...
if __name__ == "__main__":
    run_source()
    select_userdata("parquet_pipeline")